...
SIGNER=
VERSION=
url=https://github.com/linkcoin/linkcoin
proc=2
mem=2000
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/linkcoin/linkcoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/linkcoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/linkcoin/gitian.sigs.git
    git clone https://github.com/linkcoin/linkcoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/linkcoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean linkcoin git directory because of old caching
    if [ -d inputs/linkcoin/ ]; then
        echo "Cleaning Dogecoin directory..."
        rm -rf inputs/linkcoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit linkcoin="$COMMIT" --url linkcoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
