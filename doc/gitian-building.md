...
```bash
gpg --detach-sign ${VERSION}-linux/${SIGNER}/linkcoin-linux-build.assert
gpg --detach-sign ${VERSION}-win/${SIGNER}/linkcoin-win-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/linkcoin-osx-build.assert
```
...
Gitian signatures for each release are added to https://github.com/linkcoin/gitian.sigs.

`gitian-build.sh` will create signatures inside `gitian-output/sigs/` folder. Create a pull request to [linkcoin/gitian.sigs](https://github.com/linkcoin/gitian.sigs) to publish your signatures, the `.assert` and `.assert.sig` files.
