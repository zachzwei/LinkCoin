...
**Optional dependencies** :
```bash
# Qt (required for linkcoin-qt GUI)
...
According to installed dependencies, the following steps will compile `linkcoind`, `linkcoin-cli` and `linkcoin-qt`.
...
#### Enable Qt GUI
Create `linkcoin-qt`, the core wallet GUI.
```bash
./configure --with-gui
```
