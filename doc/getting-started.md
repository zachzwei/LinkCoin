...
This tutorial will help you to go through the basics to use Dogecoin Core after you completed the [installation instructions](/INSTALL.md). You now have `linkcoind` or `linkcoin-qt` executables available to run a node, and `linkcoin-cli`/`linkcoin-tx` tools to help you transact DOGE.
...
> ```console
> shibetoshi:~$ /path/to/linkcoin-cli [arguments ...]
> ```

### Table of contents

1. [Starting a linkcoin node](#starting-a-linkcoin-node)
...
To start your node, you can run an headless server using `linkcoind`:
```console
shibetoshi:~$ linkcoind -daemon
```

Or you can use the Graphical User Interface (GUI), `linkcoin-qt`:
```console
shibetoshi:~$ linkcoin-qt
```
...
> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `linkcoin-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `linkcoin-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.
...
The Dogecoin Core installation provides the `linkcoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
shibetoshi:~$ linkcoin-cli help

#Get help for a specific command
shibetoshi:~$ linkcoin-cli help COMMAND
```
...
```console
shibetoshi:~$ linkcoin-cli getaddressesbyaccount ""
[
...
```console
shibetoshi:~$ linkcoin-cli getnewaddress
...
```console
shibetoshi:~$ linkcoin-cli backupwallet /path/of/wallet/backup
```
...
```console
#Syntax
shibetoshi:~$ linkcoin-cli getbalance "*" minconf
```
...
```console
shibetoshi:~$ linkcoin-cli getbalance "*" 5
421.552000
```
...
```console
#Syntax
shibetoshi:~$ linkcoin-cli sendtoaddress address amount

#Example
shibetoshi:~$ linkcoin-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
shibetoshi:~$ linkcoin-cli listunspent minconf maxconf '["address", ...]'

#Example
shibetoshi:~$ linkcoin-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
shibetoshi:~$ linkcoin-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
shibetoshi:~$ linkcoin-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `linkcoin-cli decoderawtransaction` or `linkcoin-tx -json` to convert the content to JSON format.
...
```console
#Syntax
shibetoshi:~$ linkcoin-cli signrawtransaction encoded_transaction

#Example
shibetoshi:~$ linkcoin-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
shibetoshi:~$ linkcoin-cli sendrawtransaction signed_transaction

#Example
shibetoshi:~$ linkcoin-cli sendrawtransaction 
...
shibetoshi:~$ linkcoin-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
shibetoshi:~$ linkcoin-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
shibetoshi:~$ linkcoin-cli getrawtransaction txid verbose

#Example
shibetoshi:~$ linkcoin-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `linkcoind -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
shibetoshi:~$ linkcoind -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `linkcoin.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.linkcoin` by default, or specify the file location with `-conf`.

**linkcoin.conf example :**
```
daemon=1
server=1
listen=1
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/linkcoin.conf).
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `linkcoind -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `linkcoin-cli`.
...
You may need to specify `-datadir` also when using `linkcoin-cli`.
